...
* **Páginas Principais:**
    ...
    * `sobre.html` (Quem Somos, Missão, Visão)
    * `projetos.html` (Detalhes dos projetos sociais)
    * `voluntariado.html` (Formulário de cadastro de voluntários)
    * `contato.html` (Formulário de contato)
    * `transparencia.html` (Gráficos e prestação de contas)
    * `noticias.html` (Blog ou atualizações)
    * `parceiros.html` (Empresas e instituições que apoiam)

## Principais Tecnologias e Conceitos Aplicados

O foco desta entrega é exclusivamente a **estrutura HTML5**. O estilo (CSS) e a interatividade (JavaScript) serão adicionados em fases futuras.

* **HTML5 Semântico:** Uso correto de tags como `<header>`, `<nav>`, `<main>`, `<section>`, `<article>`, `<aside>` e `<footer>` para estruturar o conteúdo.
* **Formulários Avançados:** Criação de 4 formulários complexos com validação nativa do HTML5 (uso de `pattern`, `required`, `minlength`, etc.) para cadastro, contato e inscrição.
* **Multimídia:** Integração de elementos `<video>`, `<audio>` e galerias de imagem.
* **Imagens Responsivas:** Uso da tag `<picture>` para otimização de imagens.
* **Desempenho:** Implementação de `loading="lazy"` (Lazy Loading) para imagens.
* **Gráficos:** Uso da tag `<canvas>` para a exibição de gráficos de pizza, linha e barras na página de transparência.
